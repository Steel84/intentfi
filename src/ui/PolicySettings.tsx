...
export function PolicySettings({ config, onSave }: Props) {
  ...
  useEffect(() => {
    setSlippage(String(config.maxSlippageBps / 100));
    setImpact(String(config.maxPriceImpactBps / 100));
  }, [config.maxSlippageBps, config.maxPriceImpactBps]);
  ...
  const save = () => {
    ...
    onSave({
      ...config,
      maxSlippageBps: Math.floor(nextSlippage * 100),
      maxPriceImpactBps: Math.floor(nextImpact * 100),
    });
    setOpen(false);
  };
